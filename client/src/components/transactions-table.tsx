...
import { Search, Download, TrendingUp, TrendingDown, Edit3, Save, X, Trash2, Building2, Upload, Filter, ChevronDown, Eye, MapPin, CreditCard, Star, Receipt, ArrowUp, ArrowDown, ArrowUpDown, ChevronLeft, ChevronRight } from "lucide-react";
...
import { formatCurrency, formatDate, formatDateShort } from "@/lib/financial-utils";
import { useToast } from "@/hooks/use-toast";
import { apiRequest } from "@/lib/queryClient";
import { defaultCategories, getLedgerTypeColor, getLedgerTypeLabel } from "@/lib/transaction-classifier";
import { TransactionDetailsModal } from "./transaction-details-modal";
import { InlineCategorySelector, CategoryBadge } from "./inline-category-selector";
...
import type { Transaction, TransactionFilters, LedgerType } from "@shared/schema";
...
export default function TransactionsTable({ limit, showFilters = false, filterByCategory }: TransactionsTableProps) {
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<Partial<Transaction> & { editingCategory?: boolean }>({});
  const [categoryFilter, setCategoryFilter] = useState(filterByCategory || "");
  const [typeFilter, setTypeFilter] = useState("");
  const [selectedTransactions, setSelectedTransactions] = useState<Set<string>>(new Set());
  const [swipeStates, setSwipeStates] = useState<Record<string, { offset: number; isSwiping: boolean }>>({});
  ...
  const [pendingChanges, setPendingChanges] = useState<Partial<Transaction> | undefined>(undefined);
  ...
  const TrendIndicator = ({ merchant, amount }: { merchant: string; amount: number }) => {
    // Create consistent trend based on merchant name hash (stable, no randomness)
    const merchantHash = merchant.toLowerCase().split('').reduce((a, b) => {
  ...
  const formatCategoryDisplay = (cat: any) => {
    ...
    if (cat.subcategory && cat.subcategory.trim() && cat.subcategory !== cat.name && cat.subcategory !== 'null') {
  ...
  const merchantFilter = selectedMerchants.size > 0 ? Array.from(selectedMerchants)[0].toLowerCase() : "";
  const accountFilter = selectedAccounts.size > 0 ? Array.from(selectedAccounts)[0] : "";

  const { data: transactions, isLoading, error } = useQuery({
    queryKey: ["/api/transactions", { search: debouncedSearch, categoryFilter, typeFilter, merchantFilter, accountFilter }],
    queryFn: async () => {
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async ({ transactionIds, deleteAll }: { transactionIds?: string[], deleteAll?: boolean }) => {
    ...
    onError: () => {
      toast({ title: "Error", description: "Failed to delete manual entries", variant: "destructive" });
    },
  });
  ...
  const handleBulkDelete = (deleteAll = false) => {
    if (deleteAll) {
      ...
      if (totalCount === 0) {
        toast({ title: "No Transactions", description: "No transactions found to delete" });
        return;
      }
      if (confirm(`Are you sure you want to delete ALL ${totalCount} transactions? This cannot be undone.`)) {
        bulkDeleteMutation.mutate({ deleteAll: true });
      }
    } else {
      const selectedCount = selectedTransactions.size;
      if (selectedCount === 0) {
        toast({ title: "No Selection", description: "Please select transactions to delete" });
        return;
      }
      if (confirm(`Are you sure you want to delete ${selectedCount} selected transactions? This cannot be undone.`)) {
        bulkDeleteMutation.mutate({ transactionIds: Array.from(selectedTransactions) });
      }
    }
  };
  ...
  const toggleSelectAll = () => {
    ...
    const allSelected = currentPageTransactions.every(t => selectedTransactions.has(t.id));
    const newSelected = new Set(selectedTransactions);
  ...
  const handleDescriptionChange = (transaction: Transaction, newDescription: string) => {
  ...
  const handleCategoryChangeWithRule = (transaction: Transaction, categoryId: string, categoryName: string) => {
  ...
  const handleUpdateOnlyConfirmed = async () => {
    if (!ruleModalTransaction || !pendingChanges) {
      console.log("❌ Missing data for update:", { ruleModalTransaction, pendingChanges });
      return;
    }
    ...
    try {
      ...
      const response = await apiRequest(`/api/transactions/${ruleModalTransaction.id}`, "PATCH", updateData);
      console.log("✅ Update response:", response);
      console.log("📝 Response type:", typeof response, "Response data:", JSON.stringify(response, null, 2));
      ...
      if (error instanceof Error && (error.message.includes('Unauthorized') || error.message.includes('401'))) {
        toast({
          ...
          description: "Please log in to update transactions. Redirecting to login page...",
          variant: "destructive",
        });
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ["Date", "Merchant", "Description", "Category", "Amount", "Type", "Account"],
      ...filteredTransactions.map((t: Transaction) => [
        ...
        formatCSVField((() => {
          const account = Array.isArray(accounts) ? accounts.find((acc: any) => acc.id === t.accountId) : null;
          return account ? (account.name || account.officialName || 'Account') : 'Manual';
        })())
      ])
  ...
  const getCategoryColor = (categoryName: string) => {
    const category = categoriesArray.find((cat: any) => cat.name === categoryName);
    ...
    const categoryColors: Record<string, string> = {
      ...
      'Subscriptions & Entertainment': 'bg-violet-100 text-violet-800 border-violet-200',
  ...
  if (search) {
    ...
    filteredTransactions = filteredTransactions.filter((transaction: Transaction) =>
      ...
      (transaction.merchant && transaction.merchant.toLowerCase().includes(searchTerm))
    );
  }
  ...
  filteredTransactions = filteredTransactions.sort((a: Transaction, b: Transaction) => {
    ...
    switch (sortField) {
      case 'date':
        ...
        break;
      case 'merchant':
        comparison = (a.merchant || 'Unknown').localeCompare(b.merchant || 'Unknown');
        break;
      case 'category':
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-12">
            <p className="text-lg font-medium text-red-600">Error loading transactions</p>
            <p className="text-slate-600">{error instanceof Error ? error.message : "Please try refreshing"}</p>
          </div>
        </CardContent>
      </Card>
    );
          ...
          {showFilters && (
            <div className="flex flex-wrap items-center gap-3">
                ...
                <DialogContent className="sm:max-w-md" aria-describedby="search-description">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Enter search keyword</label>
                      <Input
                      ...
                      />
                      <p id="search-description" className="text-xs text-slate-500">
              ...
              {selectedTransactions.size > 0 && (
                <div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-md border border-blue-200 dark:border-blue-800">
                  ...
                  <Button 
                    ...
                    variant="outline" 
                    onClick={() => {
                      const selectedTxns = paginatedTransactions.filter(t => selectedTransactions.has(t.id));
                      toast({ title: "Feature Coming Soon", description: "Bulk category editing will be available soon!" });
                    }}
                    className="h-7"
                  >
              ...
              <Button 
                ...
                disabled={bulkDeleteMutation.isPending || !Array.isArray(transactions) || transactions.length === 0}
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 border-b">
            <div className="text-sm text-blue-700 dark:text-blue-300">
              ...
              {filteredTransactions.length > 0 && (
                <span className="ml-2">({filteredTransactions.length} found)</span>
              )}
              {filteredTransactions.length === 0 && (
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {incomeCategories.map((category: string) => (
                              <div key={category} className="flex items-center space-x-3 pl-2">
                                <Checkbox
              ...
              <div className="space-y-2">
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-80 max-h-60 overflow-y-auto">
                    <div className="space-y-2">
                      {Array.isArray(merchants) && merchants.map((merchant: string) => (
                    ...
                    <div className="space-y-2">
                      {Array.isArray(accounts) && accounts.map((account: any) => (
                        <div key={account.id} className="flex items-center space-x-2">
                          ...
                          />
                          <label className="text-sm">{account.displayName || account.name || account.officialName || 'Unknown Account'}</label>
                        </div>
                      ))}
                    </div>
                  </PopoverContent>
            ...
            <div className="block sm:hidden">
                ...
                <div className="flex justify-between items-center text-sm">
                  <div className="flex items-center gap-4">
                    <div className="text-green-600 dark:text-green-400 font-semibold">
                      Income: {formatCurrency(paginatedTransactions.filter(t => t.type === "income").reduce((sum, t) => sum + parseFloat(t.amount), 0))}
                    </div>
                    <div className="text-red-600 dark:text-red-400 font-semibold">
                      Expenses: {formatCurrency(paginatedTransactions.filter(t => t.type === "expense").reduce((sum, t) => sum + parseFloat(t.amount), 0))}
                    </div>
                  </div>
                  <div className="font-bold">
                    Net: {formatCurrency(paginatedTransactions.reduce((sum, t) => t.type === "income" ? sum + parseFloat(t.amount) : sum - parseFloat(t.amount), 0))}
                  </div>
                </div>
              </div>
              ...
              <div className="px-4 pb-20">
                {(() => {
                  ...
                  return Object.entries(groupedTransactions).map(([dateKey, transactions], groupIndex) => {
                    let dateLabel = dateKey;
                    if (dateKey === today) dateLabel = "Today";
                    else if (dateKey === yesterday) dateLabel = "Yesterday";
                    else {
                      const date = new Date(dateKey);
                      const isThisWeek = (Date.now() - date.getTime()) / (1000 * 60 * 60 * 24) <= 7;
                      if (isThisWeek) dateLabel = date.toLocaleDateString('en-US', { weekday: 'long' });
                      ...
                      <div key={dateKey} className="mb-6">
                        ...
                        <div className="space-y-2">
                          {transactions.map((transaction: Transaction, index: number) => {
                            ...
                            const isRecurring = transaction.merchant && ['netflix', 'spotify', 'adobe', 'disney', 'hulu', 'amazon prime', 'wells fargo'].includes(transaction.merchant.toLowerCase());
                            const categoryIcon = getCategoryIcon(transaction.category);
                            const swipeState = swipeStates[transaction.id] || { offset: 0, isSwiping: false };
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="relative flex-shrink-0">
                                    {transaction.merchant ? (
                                      <MerchantLogo merchant={transaction.merchant} size={6} />
                                    ) : (
                                    ...
                                    <div className="flex items-center justify-between">
                                      <div className="min-w-0 flex-1">
                                        ...
                                        <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                          ...
                                          {transaction.merchant && (
                                            <TrendIndicator merchant={transaction.merchant} amount={parseFloat(transaction.amount)} />
                                          )}
                                        </div>
                                      </div>
                                      ...
                                      <div className="flex items-center gap-2">
                                        <div className={`font-semibold text-xs flex-shrink-0 ${
                                          transaction.ignoreType && transaction.ignoreType !== 'none'
                ...
                <tr>
                  ...
                  <th className="px-2 sm:px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider min-w-0">
                    ...
                    >
                      ...
                      {sortField !== 'merchant' && <ArrowUpDown className="w-3 h-3 opacity-40" />}
                    </button>
                  </th>
                  ...
                  <th className="px-2 sm:px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider w-24 sm:w-auto hidden sm:table-cell">
                    ...
                    >
                      ...
                      {sortField !== 'category' && <ArrowUpDown className="w-3 h-3 opacity-40" />}
                    </button>
                  </th>
                  
                  <th className="px-2 sm:px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider w-20 sm:w-auto hidden md:table-cell">Account</th>
                  
                  {/* Sparkline Column Header */}
                  <th className="px-2 sm:px-6 py-3 text-center text-xs font-medium text-muted-foreground uppercase tracking-wider w-24 hidden sm:table-cell">Trend</th>
                  ...
                  <th className="px-2 sm:px-6 py-3 text-right text-xs font-medium text-muted-foreground uppercase tracking-wider w-28 sm:w-32">
                    ...
                    >
                      ...
                      {sortField !== 'amount' && <ArrowUpDown className="w-3 h-3 opacity-40" />}
                    </button>
                  </th>
                  <th className="px-2 sm:px-6 py-3 text-center text-xs font-medium text-muted-foreground uppercase tracking-wider w-20 sm:w-24">Actions</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={transaction.id} className="hover:bg-muted/50">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {transaction.merchant && (
                                <div className="flex-shrink-0">
                                  <MerchantLogo merchant={transaction.merchant} size={8} />
                                </div>
                              )}
                              <div className="min-w-0 flex-1">
                                {/* Merchant Name with Hover Tooltip, Click for Details, and Recurring Indicator */}
                                {transaction.merchant && (
                                  <MerchantHoverTooltip merchant={transaction.merchant}>
                                    <MerchantDetailDialog merchant={transaction.merchant}>
                                      <div className="flex items-center gap-2 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md p-1 -m-1 transition-colors" 
                                        ...
                                        {['netflix', 'spotify', 'adobe', 'disney', 'hulu', 'amazon prime'].includes(transaction.merchant.toLowerCase()) && (
                                          <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse" title="Recurring transaction"></div>
                                        )}
                                      </div>
                                ...
                                <div className={`break-words whitespace-normal leading-relaxed ${transaction.merchant ? 'text-xs text-muted-foreground' : 'font-medium'}`} 
                                     data-testid={`transaction-description-${transaction.id}`}>
                                  <InlineDescriptionEditor
                                    currentDescription={transaction.description || ''}
                                    onDescriptionChange={(newDescription) => handleDescriptionChange(transaction, newDescription)}
                                    className="w-full"
                                  />
                                </div>
                              </div>
                      ...
                      <td className="px-2 sm:px-6 py-3 sm:py-4 whitespace-nowrap hidden sm:table-cell">
                        {editingId === transaction.id && editForm.editingCategory ? (
                          <InlineCategorySelector
                            currentCategoryName={editForm.category || transaction.category}
                            onCategoryChange={(categoryId, categoryName) => {
                              // Use rule-aware handler for category changes
                              handleCategoryChangeWithRule(transaction, categoryId, categoryName);
                              setEditForm({ ...editForm, editingCategory: false });
                            }}
                      ...
                      <td className="px-2 sm:px-6 py-3 sm:py-4 whitespace-nowrap hidden md:table-cell">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            <div className="text-xs sm:text-sm font-medium text-foreground" data-testid={`account-name-${transaction.id}`}>
                              {(() => {
                                const account = Array.isArray(accounts) ? accounts.find((acc: any) => acc.id === transaction.accountId) : null;
                                return account ? (account.name || account.officialName || 'Account') : 'Manual';
                              })()}
                            </div>
                            <div className="text-xs text-muted-foreground" data-testid={`account-institution-${transaction.id}`}>
                              {(() => {
                                const account = Array.isArray(accounts) ? accounts.find((acc: any) => acc.id === transaction.accountId) : null;
                                return account ? (account.institutionName || transaction.source || 'Bank') : (transaction.source || 'Manual');
                              })()}
                            </div>
                          </div>
                        </div>
                      ...
                      <td className="px-2 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-center hidden sm:table-cell">
                        {transaction.merchant && (
                          <MerchantSparkline 
                            ...
                            transactionDate={typeof transaction.date === 'string' ? transaction.date : transaction.date.toISOString()}
                          />
                        )}
                      </td>
                ...
                return (
                  <tfoot className="bg-muted/30 border-t-2 border-border">
                    <tr className="font-semibold">
                      <td colSpan={7} className="px-2 sm:px-6 py-3 text-right text-sm">
                        Page Summary:
                      </td>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="text-sm text-muted-foreground">
                  ...
                  <span className="font-medium text-foreground">{Math.min(currentPage * pageSize, filteredTransactions.length)}</span> of{" "}
