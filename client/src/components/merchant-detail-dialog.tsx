...
export const MerchantDetailDialog = forwardRef<
  HTMLSpanElement,
  MerchantDetailDialogProps
>(({ merchant, children }, ref) => {
  ...
  const { data: merchantData, isLoading } = useQuery<any>({
    ...
    queryFn: async () => {
      ...
      const totalSpent = amounts.reduce(
      ...
      const monthlyData = Object.values(monthlyBreakdown).sort(
        (a: any, b: any) => b.month.localeCompare(a.month),
      );
      ...
      const avgDaysBetween =
        daysBetweenTransactions.length > 0
      ...
      if (avgDaysBetween >= 28 && avgDaysBetween <= 35)
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {formatCurrency(merchantData.totalSpent)}
                </div>
              </div>
              ...
              <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {formatCurrency(merchantData.avgSpend)}
                </div>
              </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  {formatDate(new Date(merchantData.firstTransaction.date))}
                </div>
                <div>
                  <span className="font-medium">Last Transaction:</span>{" "}
                  {formatDate(new Date(merchantData.lastTransaction.date))}
                </div>
                <div>
                  <span className="font-medium">Min Amount:</span>{" "}
                  {formatCurrency(merchantData.minSpend)}
                </div>
                <div>
                  <span className="font-medium">Max Amount:</span>{" "}
                  {formatCurrency(merchantData.maxSpend)}
                </div>
              </div>
            </div>
          </div>
...
MerchantDetailDialog.displayName = "MerchantDetailDialog";
